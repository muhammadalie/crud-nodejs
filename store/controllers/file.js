...
let listFiles = (req, res) => {
	...
	File.find({ userId: req.headers.username }, function(err, result) {
		if (err) return console.log(err);
		var userMap = [];
...
let deletFile = (req, res) => {
	if (req.body.clear) {
		File.deleteMany({ userId: req.body.userId }, function(err, file) {
			if (err) {
				console.log(err);
				...
				return res.send({ message: 'all files are deleted successfully!' });
			}
		});
	} else {
